...
- **The primary source for all 9 default macro variables is Trading Economics** (`te_market`/`te_indicator`); yfinance and the LSEG Data Library are configured as *fallback* sources for different subsets of variables (FRED was the fallback for 3 of them until it was deleted outright and replaced with LSEG — see CLAUDE.md's Load-bearing decisions). Exact primary/fallback pairing per variable (`macro_data.py:115-220`): DXY, VIX, IDR_USD, COAL fall back to yfinance; US_10Y, CPO, NICKEL fall back to LSEG (`US10YT=RR`, `FCPOc1`, `MNI3` respectively); BI_RATE and CHINA_PMI — which previously had **no fallback at all** — now also fall back to LSEG (`IDCBIR=ECI`, `CNPMI=ECI`), though these two economic-indicator RICs are markedly less certain than the three market-instrument ones above (unverified against a live LSEG session in this sandbox). If Trading Economics fails or `TE_API_KEY` is unset, every variable now has *some* fallback attempt rather than 2 of 9 going straight to `missing_variables`.
